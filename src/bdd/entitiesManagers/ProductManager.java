...
public class ProductManager extends Manager{
    ...
    public static ArrayList<Product> getAll(){
        ...
        try{
            ...
            while(result.next()){
                productList.add(new Product(result.getInt("id"), result.getString("designation"), result.getString("description"), result.getDouble("price")));
            }
            closeQuery();
        }catch(SQLException e){
            System.out.println(e);
    ...
    public static Product update(Product product){
        try{
            pS = connection.prepareStatement("UPDATE products SET id = ?, designation = ?, description = ?, price = ? WHERE id = ?;");
